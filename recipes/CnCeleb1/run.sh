...
if [ $stage -le 0 ]; then
  ...
  CUDA_VISIBLE_DEVICES=1,2 python -m torch.distributed.launch --nproc_per_node=2 --master_port='29501' train_speaker_embeddings.py SpeakerRec/hparams/train_ecapa_tdnn.yaml --distributed_launch --distributed_backend='nccl' --auto_mix_prec
...
if [ $stage -le 30 ]; then
  ...
  CUDA_VISIBLE_DEVICES=0,3 python -m torch.distributed.launch --master_port='29505' --nproc_per_node=2 train_speaker_embeddings.py SpeakerRec/hparams/train_resnet.yaml --distributed_launch --distributed_backend='nccl' --auto_mix_prec
