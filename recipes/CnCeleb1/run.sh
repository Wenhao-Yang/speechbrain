...
if [ $stage -le 0 ]; then
  ...
  CUDA_VISIBLE_DEVICES=2,3 MASTER_PORT=378728 python -m torch.distributed.launch --nproc_per_node=2 train_speaker_embeddings.py SpeakerRec/hparams/train_ecapa_tdnn.yaml --distributed_launch --distributed_backend='nccl'
...
if [ $stage -le 30 ]; then
  ...
  CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 train_speaker_embeddings.py SpeakerRec/hparams/train_resnet.yaml --distributed_launch --distributed_backend='nccl'
