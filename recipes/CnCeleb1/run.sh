...
if [ $stage -le 0 ]; then
  ...
  CUDA_VISIBLE_DEVICES=2 python -m torch.distributed.launch --nproc_per_node=1 train_speaker_embeddings.py SpeakerRec/hparams/train_ecapa_tdnn.yaml --distributed_launch --distributed_backend='nccl'
  exit
fi
